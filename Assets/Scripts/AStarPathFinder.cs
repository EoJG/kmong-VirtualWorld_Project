...
public class AStarPathFinder : MonoBehaviour
{
    ...
    void Update()
    {
        if ((inventory.index + 1) == (int)animal.needFood)
        {
            ...
            AStarFind(new Vector2Int(nearestGrid.nearGridIndex / 11, nearestGrid.nearGridIndex % 11), new Vector2Int(playerNearestGrid.nearGridIndex / 11, playerNearestGrid.nearGridIndex % 11));
    ...
    public void AStarFind(Vector2Int start, Vector2Int goal)
    {
        PriorityQueue<Vector2Int> pq = new();
        ...
        while (pq.Count > 0)
        {
            ...
            foreach (Vector2Int dir in directions)
            {
                ...
                if (InBounds(next) && grids[next.x, next.y])
                {
                    float newG = gScore[current] + 1f;

                    if (!gScore.ContainsKey(next) || newG < gScore[next])
                    {
                        gScore[next] = newG;
